...
void BFS_path_length(int source){
     queue<int>q;
     q.push(source);
     while(!q.empty()){
        int x=q.front();
...
int BFS_print_path(int starting_point,int ending_point){
   ...
   q1.push(parent);
   BFS_print_path(starting_point,parent);
...
int main(){
    ...
    printf("Destination : ");
    scanf("%d",&destination);
    BFS_path_length(source);
    BFS_print_path(source,destination);
    printf("\n");
